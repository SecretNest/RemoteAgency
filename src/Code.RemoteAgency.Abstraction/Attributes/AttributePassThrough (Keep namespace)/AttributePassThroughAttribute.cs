...
namespace SecretNest.RemoteAgency.Attributes
{
    ...
    public sealed class AttributePassThroughAttribute : Attribute
    {
        ...
        /// <para>When named parameter is required for constructing, sets types of full form parameters of constructor to <paramref name="attributeConstructorParameterTypes"/> and leading values to <paramref name="attributeConstructorParameters"/>. Marks the rest one by one using <see cref="AttributePassThroughIndexBasedParameterAttribute"/>. The <see cref="AttributePassThroughAdditionalDataAttributeBase.AttributeId"/> should have the same value as <paramref name="attributeId"/>.</para>
        /// <para>When properties need to be set while initializing, marks the properties one by one using <see cref="AttributePassThroughPropertyAttribute"/>. The <see cref="AttributePassThroughAdditionalDataAttributeBase.AttributeId"/> should have the same value as <paramref name="attributeId"/>.</para>
        /// <para>When fields need to be set while initializing, marks the fields one by one using <see cref="AttributePassThroughFieldAttribute"/>. The <see cref="AttributePassThroughAdditionalDataAttributeBase.AttributeId"/> should have the same value as <paramref name="attributeId"/>.</para>
        /// </remarks>
